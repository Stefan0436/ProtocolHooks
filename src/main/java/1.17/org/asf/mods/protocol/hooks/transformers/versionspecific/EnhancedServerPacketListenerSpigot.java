...
public class EnhancedServerPacketListenerSpigot {
	...
	public void internalTeleport(double x, double y, double z, float xR, float yR, Set<RelativeArgument> set, boolean b) {
	...
	public void handleChat(
			@TargetType(target = "net.minecraft.server.network.TextFilter$FilteredText") FilteredText txt,
			@LocalVariable String message) {
		if (!message.startsWith("/")) {
			...
			if (obj.getFormatter() != null) {
				String filteredString = txt.getFiltered();
				Component rawComp = obj.getFormatter().format(message, EnhancedPlayer.from(player));
				Component filteredComp = obj.getFormatter().format(filteredString, EnhancedPlayer.from(player));
				server.getPlayerList().broadcastMessage(rawComp, other -> {
					if (player.shouldFilterMessageTo(other))
						return filteredComp;
					else
						return rawComp;
